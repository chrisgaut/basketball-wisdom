...
get_team_advanced_stats_bref <- function(year) {
  ...
  df <- df %>%
    mutate(Age = as.numeric(Age),
           Wins = as.numeric(W),
           Losses = as.numeric(L),
           `Pythag. Wins` = as.numeric(PW),
           `Pythag. Losses` = as.numeric(PL),
           `Margin of Victory` = as.numeric(MOV),
           `Strength of Schedule` = as.numeric(SOS),
           `Simple Rating System` = as.numeric(SRS),
           `Offensive Rating` = as.numeric(ORtg),
           `Defensive Rating` = as.numeric(DRtg),
           `Net Rating` = as.numeric(NRtg),
           Pace = as.numeric(Pace),
           `FT Rate` = as.numeric(FTr),
           `3PA Rate` = as.numeric(`3PAr`),
           `True Shooting %` = as.numeric(`TS%`),
           `Effective FG %` = as.numeric(`eFG%`),
           `Turnover %` = as.numeric(`TOV%`),
           `Offensive Rebound %` = as.numeric(`ORB%`),
           `FT/FGA` = as.numeric(`FT/FGA`),
           `Opp. Effective FG %` = as.numeric(`Opp. eFG%`),
           `Opp. Turnover %` = as.numeric(`Opp. TOV%`),
           `Defensive Rebound %` = as.numeric(`DRB%`),
